<template>
  <div class="rent-properties-page">
    <!-- Hero Section -->
    <section class="hero-section">
      <div class="hero-content">
        <h1>Find Your Perfect Rental</h1>
        <p>Discover properties available for rent in prime locations</p>
      </div>
    </section>
...
const filteredProperties = computed(() => {
  let properties = propertyStore.properties.filter(p => p.status === 'For Rent');
  ...
  if (filters.value.rentRange) {
    const [min, max] = filters.value.rentRange.split('-').map(Number);
    properties = properties.filter(p => {
...
.hero-section {
  background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?ixlib=rb-4.0.3');
